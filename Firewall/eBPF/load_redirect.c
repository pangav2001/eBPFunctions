...
int main(int argc, char **argv)
{
    ...
    bprog = bpf_object_find_program_by_name(bobj, "xdp_red");

    bprog_fd = bpf_program_fd(bprog);

    bpf_xdp_attach_SKB_simple(ifindex, bprog_fd);

    // bpf_object_pin_maps(bobj, "/sys/fs/bpf/xdp_red/");
...
// clang -Wall -I ../../libbpf/include -o load_redirect.out load_redirect.c -L ../../libbpf/src -lbpf