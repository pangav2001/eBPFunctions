...
int main(int argc, char **argv)
{
    ...
    bprog = bpf_object_find_program_by_name(bobj, "xdp_lpm_test");

    bprog_fd = bpf_program_fd(bprog);

    bpf_xdp_attach_SKB_simple(ifindex, bprog_fd);

    bpf_object_pin_maps(bobj, "/sys/fs/bpf/xdp_lpm_test/");
...
// clang -Wall -I ../../libbpf/include -o load_lpm_trie.out load_lpm_trie.c -L ../../libbpf/src -lbpf