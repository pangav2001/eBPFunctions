...
int main(int argc, char **argv)
{
    ...
    reuse_pinned_maps(bobj, "/sys/fs/bpf/xdp_lpm_test/");
    ...
    struct ipv4_lpm_key key = {
                .prefixlen = 32,
                .data = get_ip_address_in_nbo(FORBIDDEN_SRC_IP)
        };
    bool to_add = false;
    int fd = bpf_object_find_map_fd_by_name(bobj, "lpm_trie");
    bpf_map_update_elem(fd, &key, &to_add, BPF_ANY);
    bpf_object__close(bobj);

    return 0;
}

// clang -Wall -I ../../libbpf/include -o add_lpm_rules.out add_lpm_rules.c -L ../../libbpf/src -lbpf